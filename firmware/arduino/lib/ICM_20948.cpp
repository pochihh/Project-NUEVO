...
#include <cmath>    // for atan2, asin, sqrt, M_PI
...
// SPI not implemented for Linux version
...
void ICM_20948::enableDebugging(DebugStream &debugPort)
{
...
{
  static StdDebugStream defaultStream;
  _debugSerial = &defaultStream;
  _printDebug = true;
}
...
{
  doDebugPrint(line, false);
}

void ICM_20948::debugPrintln(const char *line)
{
  doDebugPrint(line, true);
}

void ICM_20948::doDebugPrint(const char *string, bool newLine)
{
  if (_printDebug == false || _debugSerial == nullptr)
    return; // Bail if debugging is not enabled

  _debugSerial->print(string);
...
ICM_20948_Status_e ICM_20948::setSampleMode(uint8_t sensor_id_bm, uint8_t lp_config_cycle_mode)
{
  ...
  usleep(1000); // Give the ICM20948 time to change the sample mode (see issue #8) - 1ms
  return status;
}
...
ICM_20948::EulerAngles ICM_20948::quaternionToEuler(float qw, float qx, float qy, float qz) const
{
  EulerAngles euler;
  ...
  if (fabs(sinp) >= 1.0f) {
    euler.pitch = copysign(M_PI / 2.0f, sinp) * 180.0f / M_PI; // Use 90 degrees if out of range
  } else {
...
ICM_20948::EulerAngles ICM_20948::dmpQuat9ToEuler(const icm_20948_DMP_data_t &data) const
{
  // Extract quaternion from DMP data
  float q1 = ((float)data.Quat9.Data.Q1) / 1073741824.0f; // Convert Q30 to float
  float q2 = ((float)data.Quat9.Data.Q2) / 1073741824.0f;
  float q3 = ((float)data.Quat9.Data.Q3) / 1073741824.0f;
  
  // Calculate q0 from q1, q2, q3 (q0^2 = 1 - q1^2 - q2^2 - q3^2)
  ...
  // Convert to Euler angles (quaternion order is W, X, Y, Z)
  return quaternionToEuler(q0, q1, q2, q3);
}
...
ICM_20948_Status_e ICM_20948_I2C::begin(LinuxI2C &wirePort, bool ad0val, uint8_t ad0pin)
{
  // Associate
  _ad0 = ad0pin;
  _i2c = &wirePort;
  ...
  // Note: AD0 pin control not needed for Linux - address is determined by hardware
...
ICM_20948_Status_e ICM_20948_I2C::begin(const std::string& device, bool ad0val)
{
  static LinuxI2C defaultI2C;
  if (!defaultI2C.begin(device)) {
    return ICM_20948_Stat_Err;
  }
  return begin(defaultI2C, ad0val);
}
...
ICM_20948_Status_e ICM_20948_write_I2C(uint8_t reg, uint8_t *data, uint32_t len, void *user)
{
  ...
  LinuxI2C *_i2c = ((ICM_20948_I2C *)user)->_i2c; // Cast user field to ICM_20948_I2C type and extract the I2C interface pointer
...
ICM_20948_Status_e ICM_20948_read_I2C(uint8_t reg, uint8_t *buff, uint32_t len, void *user)
{
  ...
  LinuxI2C *_i2c = ((ICM_20948_I2C *)user)->_i2c;
  uint8_t addr = ((ICM_20948_I2C *)user)->_addr;
  if (_i2c == NULL)
  {
    return ICM_20948_Stat_ParamErr;
