...
Wrapper around ICM-20948 library (in `src/lib/`).
...
│       │   ├── ICM_20948.h         # IMU driver header
│       │   ├── PCA9685.cpp/h       # PWM servo driver
│       │   └── Adafruit_NeoPixel.h # NeoPixel header (uses global lib)
│       │
...
│       │   ├── UARTDriver.h/cpp    # TLV UART communication wrapper
...
**Layer Separation:**
- `src/lib/`: External libraries, not modified (or minimally patched)
...
- `tests/`: Each test in its own folder with symbolic link to src/ (includes lib/)
...
Each test folder needs a symbolic link to share the src/ directory with the main sketch (which includes src/lib/):
...
| **5** | `test_voltage.ino` | ✅ | 2026-01-28 | Tested |
| **5** | `test_buttons.ino` | ✅ | 2026-01-28 | Tested |
| **5** | `test_leds.ino` | ✅ | 2026-01-28 | Tested |
