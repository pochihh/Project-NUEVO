...
- **Pinout per driver:**
  ...
  - GND
  - Vref (current limit adjustment potentiometer on driver module)
...
Each driver module has an on-board Vref potentiometer for current limiting. Current limit must be set based on motor specifications to prevent damage.

**A4988 Current Limit Formula:**
```
I_limit (A) = Vref (V) / (8 × Rsense)

For standard A4988 with Rsense = 0.1Ω:
I_limit (A) = Vref (V) / 0.8

Example: For 1.2A motor current → Vref = 1.2A × 0.8 = 0.96V
```

**DRV8825 Current Limit Formula:**
```
I_limit (A) = Vref (V) / (5 × Rsense)

For standard DRV8825 with Rsense = 0.1Ω:
I_limit (A) = Vref (V) / 0.5

Example: For 1.2A motor current → Vref = 1.2A × 0.5 = 0.60V
```

**Recommended Current Setting:**
- Set current limit to **70-80% of motor rated current**
- Allows thermal margin and prevents overheating
- Example: 1.5A rated motor → set to 1.2A (80%)

**Common NEMA Stepper Current Settings:**

| Motor Model | Voltage | Rated Current | Target Current | A4988 Vref | DRV8825 Vref |
|-------------|---------|---------------|----------------|------------|--------------|
| NEMA 17 (42BYGHW609) | 5V | 1.5A | 1.2A | 0.96V | 0.60V |
| NEMA 17 (17HS4401) | 12V | 1.68A | 1.3A | 1.04V | 0.65V |
| NEMA 17 (17HS8401) | 12V | 1.2A | 1.0A | 0.80V | 0.50V |
| NEMA 14 (35BYGHM) | 5V | 0.8A | 0.64A | 0.51V | 0.32V |

**Arduino Pin Allocation (Steppers):**
- Stepper 1: STEP1 (pin 14), DIR1 (pin 22), EN1 (pin 26)
- Stepper 2: STEP2 (pin 15), DIR2 (pin 23), EN2 (pin 27)
- Stepper 3: STEP3 (pin 32), DIR3 (pin 24), EN3 (pin 28)
- Stepper 4: STEP4 (pin 33), DIR4 (pin 25), EN4 (pin 29)
